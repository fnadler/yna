...
/* PRO-00 — Apresentação da plataforma (3 slides conceituais + argumentos
   para o profissional). Replica o layout/imagens do Ben00Apresentacao, com
   copy calibrada (Sábio + Criador + Herói — par a par). */

const slides = [
  {
    image: '/images/slide1-pro.png',
    eyebrow: 'Por que a YNA',
    title: 'Atenda com autonomia e respaldo',
    highlight: 'autonomia e respaldo',
    body: 'Você cuida; a YNA cuida do resto — agenda, prontuário, pagamentos e supervisão clínica.',
  },
  {
    image: '/images/slide2-pro.png',
...
export function Pro00Apresentacao() {
  ...
  const handleNext = () => {
    if (isLast) navigate('/pro/cadastro')
    else navigate(`/pro/apresentacao/${step + 1}`)
  }
    ...
    <OnboardingSplit panel={imagePanel}>
      ...
      >
        ...
        <div className="mt-10 md:mt-0 flex flex-col gap-4 animate-yna-slide-up animate-yna-delay-550">
          ...
          {!isLast && (
            <Button variant="ghost" fullWidth onClick={() => navigate('/pro/cadastro')}>
              Pular apresentação
            </Button>
          )}
        </div>
