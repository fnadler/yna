...
type ButtonVariant = 'primary' | 'secondary' | 'soft' | 'ghost' | 'danger' | 'link'
...
const variantClasses: Record<ButtonVariant, string> = {
  primary:
    'bg-primary text-white hover:bg-primary-600 active:translate-y-0 hover:-translate-y-px hover:shadow-sm',
  secondary:
...
export function Button({
  ...
  return (
    <button
      className={`inline-flex items-center justify-center whitespace-nowrap rounded-pill font-semibold leading-none tracking-[-0.005em] transition-all duration-150 disabled:pointer-events-none disabled:opacity-50 ${variantClasses[variant]} ${sizeClasses[size]} ${fullWidth ? 'w-full' : ''} ${className}`}
      {...rest}
    >
